...
GST_DEBUG_CATEGORY_STATIC (gst_grokjpeg2000_enc_debug);
#define GST_CAT_DEFAULT gst_grokjpeg2000_enc_debug

#define GST_GROKJPEG2000_ENC_TYPE_PROGRESSION_ORDER (gst_grokjpeg2000_enc_progression_order_get_type())
static GType
gst_grokjpeg2000_enc_progression_order_get_type (void)
{
  ...
  if (g_once_init_enter ((gsize *) & id)) {
    ...
    _id = g_enum_register_static ("GstGrokJPEG2000EncProgressionOrder", values);
...
static void gst_grokjpeg2000_enc_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec);
static void gst_grokjpeg2000_enc_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec);

static gboolean gst_grokjpeg2000_enc_start (GstVideoEncoder * encoder);
static gboolean gst_grokjpeg2000_enc_stop (GstVideoEncoder * encoder);
static gboolean gst_grokjpeg2000_enc_set_format (GstVideoEncoder * encoder,
    GstVideoCodecState * state);
static GstFlowReturn gst_grokjpeg2000_enc_handle_frame (GstVideoEncoder *
    encoder, GstVideoCodecFrame * frame);
static gboolean gst_grokjpeg2000_enc_propose_allocation (GstVideoEncoder *
...
static GstStaticPadTemplate gst_grokjpeg2000_enc_sink_template =
...
static GstStaticPadTemplate gst_grokjpeg2000_enc_src_template =
...
#define parent_class gst_grokjpeg2000_enc_parent_class
G_DEFINE_TYPE (GstGrokJPEG2000Enc, gst_grokjpeg2000_enc,
    GST_TYPE_VIDEO_ENCODER);

static void
gst_grokjpeg2000_enc_class_init (GstGrokJPEG2000EncClass * klass)
{
  ...
  gobject_class->set_property = gst_grokjpeg2000_enc_set_property;
  gobject_class->get_property = gst_grokjpeg2000_enc_get_property;
  ...
  g_object_class_install_property (gobject_class, PROP_PROGRESSION_ORDER,
      g_param_spec_enum ("progression-order", "Progression Order",
          "Progression order", GST_GROKJPEG2000_ENC_TYPE_PROGRESSION_ORDER,
  ...
  gst_element_class_add_static_pad_template (element_class,
      &gst_grokjpeg2000_enc_src_template);
  gst_element_class_add_static_pad_template (element_class,
      &gst_grokjpeg2000_enc_sink_template);

  gst_element_class_set_static_metadata (element_class,
      "Grok JPEG2000 encoder",
      "Codec/Encoder/Video",
      "Encode JPEG2000 streams",
      "Sebastian Dröge <sebastian.droege@collabora.co.uk>");

  video_encoder_class->start = GST_DEBUG_FUNCPTR (gst_grokjpeg2000_enc_start);
  video_encoder_class->stop = GST_DEBUG_FUNCPTR (gst_grokjpeg2000_enc_stop);
  video_encoder_class->set_format =
      GST_DEBUG_FUNCPTR (gst_grokjpeg2000_enc_set_format);
  video_encoder_class->handle_frame =
      GST_DEBUG_FUNCPTR (gst_grokjpeg2000_enc_handle_frame);
  video_encoder_class->propose_allocation =
      gst_grokjpeg2000_enc_propose_allocation;

  GST_DEBUG_CATEGORY_INIT (gst_grokjpeg2000_enc_debug, "grokjpeg2000enc", 0,
      "Grok Encoder");
}

static void
gst_grokjpeg2000_enc_init (GstGrokJPEG2000Enc * self)
{
...
static void
gst_grokjpeg2000_enc_set_property (GObject * object, guint prop_id,
    const GValue * value, GParamSpec * pspec)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (object);
...
static void
gst_grokjpeg2000_enc_get_property (GObject * object, guint prop_id,
    GValue * value, GParamSpec * pspec)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (object);
...
static gboolean
gst_grokjpeg2000_enc_start (GstVideoEncoder * encoder)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (encoder);
...
static gboolean
gst_grokjpeg2000_enc_stop (GstVideoEncoder * video_encoder)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (video_encoder);
...
static gboolean
gst_grokjpeg2000_enc_set_format (GstVideoEncoder * encoder,
    GstVideoCodecState * state)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (encoder);
...
static opj_image_t *
gst_grokjpeg2000_enc_fill_image (GstGrokJPEG2000Enc * self,
    GstVideoFrame * frame)
{
  gint i, ncomps;
...
static void
gst_grokjpeg2000_enc_opj_error (const char *msg, void *userdata)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (userdata);
  gchar *trimmed = g_strchomp (g_strdup (msg));
  GST_TRACE_OBJECT (self, "grokjpeg2000 error: %s", trimmed);
  g_free (trimmed);
}

static void
gst_grokjpeg2000_enc_opj_warning (const char *msg, void *userdata)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (userdata);
  gchar *trimmed = g_strchomp (g_strdup (msg));
  GST_TRACE_OBJECT (self, "grokjpeg2000 warning: %s", trimmed);
  g_free (trimmed);
}

static void
gst_grokjpeg2000_enc_opj_info (const char *msg, void *userdata)
{
  GstGrokJPEG2000Enc *self = GST_GROKJPEG2000_ENC (userdata);
  gchar *trimmed = g_strchomp (g_strdup (msg));
  GST_TRACE_OBJECT (self, "grokjpeg2000 info: %s", trimmed);
  g_free (trimmed);
}
...
gst_grokjpeg2000_enc_handle_frame (GstVideoEncoder * encoder,
    GstVideoCodecFrame * frame)
{
  ...
  if (G_UNLIKELY (gst_debug_category_get_threshold (GST_CAT_DEFAULT) >=
          GST_LEVEL_TRACE)) {
    opj_set_info_handler (enc, gst_grokjpeg2000_enc_opj_info, self);
    opj_set_warning_handler (enc, gst_grokjpeg2000_enc_opj_warning, self);
    opj_set_error_handler (enc, gst_grokjpeg2000_enc_opj_error, self);
  } else {
  ...
  image = gst_grokjpeg2000_enc_fill_image (self, &vframe);
  if (!image)
...
initialization_error:
  {
    ...
    GST_ELEMENT_ERROR (self, LIBRARY, INIT,
        ("Failed to initialize Grok encoder"), (NULL));
...
fill_image_error:
  {
    ...
    GST_ELEMENT_ERROR (self, LIBRARY, INIT,
        ("Failed to fill Grok image"), (NULL));
    return GST_FLOW_ERROR;
  }
open_error:
  {
...
encode_error:
  {
    ...
    GST_ELEMENT_ERROR (self, STREAM, ENCODE,
        ("Failed to encode Grok stream"), (NULL));
    return GST_FLOW_ERROR;
  }
}

static gboolean
gst_grokjpeg2000_enc_propose_allocation (GstVideoEncoder * encoder,
    GstQuery * query)
{
