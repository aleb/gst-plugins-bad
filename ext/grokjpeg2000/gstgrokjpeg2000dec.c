...
GST_DEBUG_CATEGORY_STATIC (gst_grokjpeg2000_dec_debug);
#define GST_CAT_DEFAULT gst_grokjpeg2000_dec_debug

static gboolean gst_grokjpeg2000_dec_start (GstVideoDecoder * decoder);
static gboolean gst_grokjpeg2000_dec_stop (GstVideoDecoder * decoder);
static gboolean gst_grokjpeg2000_dec_set_format (GstVideoDecoder * decoder,
    GstVideoCodecState * state);
static GstFlowReturn gst_grokjpeg2000_dec_handle_frame (GstVideoDecoder *
    decoder, GstVideoCodecFrame * frame);
static gboolean gst_grokjpeg2000_dec_decide_allocation (GstVideoDecoder *
...
static GstStaticPadTemplate gst_grokjpeg2000_dec_sink_template =
...
static GstStaticPadTemplate gst_grokjpeg2000_dec_src_template =
...
#define parent_class gst_grokjpeg2000_dec_parent_class
G_DEFINE_TYPE (GstGrokJPEG2000Dec, gst_grokjpeg2000_dec,
    GST_TYPE_VIDEO_DECODER);

static void
gst_grokjpeg2000_dec_class_init (GstGrokJPEG2000DecClass * klass)
{
  ...
  gst_element_class_add_static_pad_template (element_class,
      &gst_grokjpeg2000_dec_src_template);
  gst_element_class_add_static_pad_template (element_class,
      &gst_grokjpeg2000_dec_sink_template);

  gst_element_class_set_static_metadata (element_class,
      "Grok JPEG2000 decoder",
      "Codec/Decoder/Video",
      "Decode JPEG2000 streams",
      "Sebastian Dröge <sebastian.droege@collabora.co.uk>");

  video_decoder_class->start = GST_DEBUG_FUNCPTR (gst_grokjpeg2000_dec_start);
  video_decoder_class->stop = GST_DEBUG_FUNCPTR (gst_grokjpeg2000_dec_stop);
  video_decoder_class->set_format =
      GST_DEBUG_FUNCPTR (gst_grokjpeg2000_dec_set_format);
  video_decoder_class->handle_frame =
      GST_DEBUG_FUNCPTR (gst_grokjpeg2000_dec_handle_frame);
  video_decoder_class->decide_allocation =
      gst_grokjpeg2000_dec_decide_allocation;

  GST_DEBUG_CATEGORY_INIT (gst_grokjpeg2000_dec_debug, "grokjpeg2000dec", 0,
      "Grok Decoder");
}

static void
gst_grokjpeg2000_dec_init (GstGrokJPEG2000Dec * self)
{
...
static gboolean
gst_grokjpeg2000_dec_start (GstVideoDecoder * decoder)
{
  GstGrokJPEG2000Dec *self = GST_GROKJPEG2000_DEC (decoder);
...
static gboolean
gst_grokjpeg2000_dec_stop (GstVideoDecoder * video_decoder)
{
  GstGrokJPEG2000Dec *self = GST_GROKJPEG2000_DEC (video_decoder);
...
static gboolean
gst_grokjpeg2000_dec_set_format (GstVideoDecoder * decoder,
    GstVideoCodecState * state)
{
  GstGrokJPEG2000Dec *self = GST_GROKJPEG2000_DEC (decoder);
  GstStructure *s;
...
gst_grokjpeg2000_dec_negotiate (GstGrokJPEG2000Dec * self, opj_image_t * image)
{
...
static void
gst_grokjpeg2000_dec_opj_error (const char *msg, void *userdata)
{
  GstGrokJPEG2000Dec *self = GST_GROKJPEG2000_DEC (userdata);
...
static void
gst_grokjpeg2000_dec_opj_warning (const char *msg, void *userdata)
{
  GstGrokJPEG2000Dec *self = GST_GROKJPEG2000_DEC (userdata);
  gchar *trimmed = g_strchomp (g_strdup (msg));
...
static void
gst_grokjpeg2000_dec_opj_info (const char *msg, void *userdata)
{
  GstGrokJPEG2000Dec *self = GST_GROKJPEG2000_DEC (userdata);
  gchar *trimmed = g_strchomp (g_strdup (msg));
...
static GstFlowReturn
gst_grokjpeg2000_dec_handle_frame (GstVideoDecoder * decoder,
    GstVideoCodecFrame * frame)
{
  ...
  if (G_UNLIKELY (gst_debug_category_get_threshold (GST_CAT_DEFAULT) >=
          GST_LEVEL_TRACE)) {
    opj_set_info_handler (dec, gst_grokjpeg2000_dec_opj_info, self);
    opj_set_warning_handler (dec, gst_grokjpeg2000_dec_opj_warning, self);
    opj_set_error_handler (dec, gst_grokjpeg2000_dec_opj_error, self);
  } else {
  ...
  ret = gst_grokjpeg2000_dec_negotiate (self, image);
  if (ret != GST_FLOW_OK)
...
initialization_error:
  {
    ...
    GST_ELEMENT_ERROR (self, LIBRARY, INIT,
        ("Failed to initialize Grok decoder"), (NULL));
    return GST_FLOW_ERROR;
  }
map_read_error:
  {
...
open_error:
  {
    ...
    GST_ELEMENT_ERROR (self, LIBRARY, INIT,
        ("Failed to open Grok stream"), (NULL));
    return GST_FLOW_ERROR;
  }
decode_error:
  {
    ...
    GST_VIDEO_DECODER_ERROR (self, 1, STREAM, DECODE,
        ("Failed to decode Grok stream"), (NULL), ret);
    return ret;
  }
negotiate_error:
  {
...
static gboolean
gst_grokjpeg2000_dec_decide_allocation (GstVideoDecoder * decoder,
    GstQuery * query)
{
  GstBufferPool *pool;
  GstStructure *config;
